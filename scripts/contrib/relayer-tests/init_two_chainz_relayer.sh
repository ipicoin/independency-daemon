#!/bin/bash
# init_two_chainz_relayer creates two ipid chains and configures the relayer
...
if ! [ -x "$(which rly)" ]; then
  echo "Error: ipid is not installed. Try running 'make install'" >&2
  exit 1
fi

# Ensure ipid is installed
if ! [ -x "$(which ipid)" ]; then
  echo "Error: ipid is not installed. Try running 'make install'" >&2
  exit 1
fi
...
# Stop existing ipid processes
killall ipid &> /dev/null
...
echo "Generating ipid configurations..."
mkdir -p $WASMD_DATA && cd $WASMD_DATA && cd ../
./one_chain.sh ipid $chainid0 ./data 26657 26656 6060 9090
./one_chain.sh ipid $chainid1 ./data 26557 26556 6061 9091
...
rly config init
rly config add-chains configs/ipid/chains
rly config add-paths configs/ipid/paths
